TARGET_REPO := ~/Projects/kommander
...
RSYNC_OPTS := -rav --delete --exclude .idea --exclude .local --exclude .trunck --exclude bin --exclude dist -e "ssh $(SSH_OPTS)" $(TARGET_REPO) $(EC2_INSTANCE_USER)@$(EC2_INSTANCE_HOST):~/go/src/github.com/mesosphere
